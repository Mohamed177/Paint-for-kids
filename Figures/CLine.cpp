...
bool CLine::ValidMove(Point p, bool scramble = 0) 
{
	...
	if ((v1.y > UI.ToolBarHeight && v2.y > UI.ToolBarHeight && v1.y < (UI.height - UI.StatusBarHeight) && v2.y < (UI.height - UI.StatusBarHeight) && v1.x <=( UI.width-13 - (FigGfxInfo.BorderWdth/2))  &&v2.x <= (UI.width - 13 - (FigGfxInfo.BorderWdth / 2)) && v1.x>=left_border &&v2.x >=left_border))
	{
		return true;
	}
	return false;
